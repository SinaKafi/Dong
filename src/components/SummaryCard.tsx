...
export function SummaryCard({ result, currency, children }: SummaryCardProps) {
  const showGlobalShare = result.allSharedByAll;
  const shareText = showGlobalShare
    ? result.remainder > 0
    ...
    : "سهم هر فرد بر اساس هزینه‌هایی که در آن حضور داشته محاسبه شده است.";
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
          ...
          <p className="mt-1 text-sm font-semibold leading-6 text-white">
            {showGlobalShare ? <span className="font-mono tabular-nums">{shareText}</span> : shareText}
          </p>
        </div>
      </div>
